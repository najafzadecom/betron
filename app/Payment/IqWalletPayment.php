...
class IqWalletPayment
{
    ...
    {
        $this->baseUrl = config('iqwallet.endpoint');
        $this->email = config('iqwallet.email');
        $this->key = config('iqwallet.key');
    }
    ...
    public function getTransactionHistory(int $minuteRange = 60, ?string $iban = null): array
    {
        ...
        try {
            ...
            Log::channel('iqwallet')->error('IQ Wallet transaction history error: ' . $e->getMessage());
    ...
    public function sendWithdrawal(array $data): array
    {
        ...
        try {
            ...
            Log::channel('iqwallet')->error('IQ Wallet withdrawal error: ' . $e->getMessage());
    ...
    public function checkWithdrawalStatus(int $minuteRange = 10, ?string $referenceId = null): array
    {
        ...
        try {
            ...
            Log::channel('iqwallet')->error('IQ Wallet withdrawal status error: ' . $e->getMessage());
    ...
    public function getWithdrawalAccounts(): array
    {
        ...
        try {
            ...
            Log::channel('iqwallet')->error('IQ Wallet accounts error: ' . $e->getMessage());
    ...
    public function getWithdrawalReceipt(string $uniqueId): array
    {
        ...
        try {
            ...
            Log::channel('iqwallet')->error('IQ Wallet receipt error: ' . $e->getMessage());
    ...
    private function handleResponse(Response $response): array
    {
        if ($response->successful()) {
            ...
            if ($data['status'] ?? false) {
                ...
                Log::channel('iqwallet')->warning('IQ Wallet API error: ', $data);
        ...
        Log::channel('iqwallet')->error('IQ Wallet HTTP error: ' . $response->status() . ' - ' . $response->body());
