            ...
            <div class="card-body">
                <div class="alert alert-info mb-4">
                    ...
                    @if($withdrawal->withdrawal_id)
                        <br><strong>{{ __('Operation ID') }}:</strong> {{ $withdrawal->withdrawal_id }}
                    @endif
                </div>
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="card border">
                                <div class="card-header bg-light">
                                    <h6 class="mb-0">{{ __('Withdrawal Information') }}</h6>
                                </div>
                                <div class="card-body">
                                    <div class="row mb-2">
                                        <div class="col-5 fw-semibold text-muted">{{ __('Withdrawal ID') }}:</div>
                                        <div class="col-7 text-end">{{ $paypapStatus['withdrawalId'] ?? '-' }}</div>
                                    </div>
                                    <div class="row mb-2">
                                        <div class="col-5 fw-semibold text-muted">{{ __('Status') }}:</div>
                                        <div class="col-7 text-end">
                                            @if(isset($paypapStatus['status']))
                                                <span class="badge bg-{{ $paypapStatus['status']['code'] == 4001 ? 'success' : 'warning' }}">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-7 text-end">{{ \Carbon\Carbon::parse($paypapStatus['createdAt'])->format('Y-m-d H:i:s') }}</div>
                                    </div>
                                    @endif
                                    @if(isset($paypapStatus['updatedAt']))
                                    <div class="row mb-2">
                                        <div class="col-5 fw-semibold text-muted">{{ __('Updated At') }}:</div>
                                        <div class="col-7 text-end">{{ \Carbon\Carbon::parse($paypapStatus['updatedAt'])->format('Y-m-d H:i:s') }}</div>
                                    </div>
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="card-body">
                                    @if(isset($paypapStatus['user']))
                                        <div class="row mb-2">
                                            ...
                                            <div class="col-7 text-end">{{ $paypapStatus['user']['userId'] ?? '-' }}</div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-5 fw-semibold text-muted">{{ __('Username') }}:</div>
                                            <div class="col-7 text-end">{{ $paypapStatus['user']['username'] ?? '-' }}</div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-5 fw-semibold text-muted">{{ __('Full Name') }}:</div>
                                            <div class="col-7 text-end">{{ $paypapStatus['user']['fullName'] ?? '-' }}</div>
                                        </div>
                                    @else
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="card border">
                                ...
                                <div class="card-body">
                                    <pre class="bg-light p-3 rounded" style="max-height: 400px; overflow-y: auto;">{{ json_encode($paypapStatus, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                                </div>
                            </div>
                        </div>
                    </div>
